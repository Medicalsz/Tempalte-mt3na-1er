...
public class PatientCommandesController {
    ...
    private void addRows(List<Commande> commandes) {
        ...
        for (int i = 0; i < commandes.size(); i++) {
            ...
            status.setStyle("-fx-font-size: 11px; -fx-text-fill: white; -fx-padding: 3 10; -fx-background-color: " + statusColor(c.getStatus()) + "; -fx-background-radius: 12;");
            ...
            btnCancel.setDisable(!"en_attente".equalsIgnoreCase(c.getStatus()) && !"confirmee".equalsIgnoreCase(c.getStatus()) && !"en_attente_paiement".equalsIgnoreCase(c.getStatus()));
    ...
    private void showEditForm(Commande c) {
        ...
        Runnable recompute = () -> {
            if (Validators.isStrictlyPositiveInt(qtyField.getText())) {
                ...
                BigDecimal t = (product.getPrice() != null ? product.getPrice() : BigDecimal.ZERO).multiply(BigDecimal.valueOf(q));
        ...
        btnSave.setStyle("-fx-background-color: #1a73e8; -fx-text-fill: white; -fx-font-size: 13px; -fx-background-radius: 8; -fx-cursor: hand; -fx-padding: 9 24;");
        Button btnCancel = new Button("Annuler");
        btnCancel.setStyle("-fx-background-color: #e5e7eb; -fx-text-fill: #333; -fx-font-size: 13px; -fx-background-radius: 8; -fx-cursor: hand; -fx-padding: 9 24;");
        ...
        btnSave.setOnAction(e -> {
            ...
            c.setQuantity(qty);
            c.setTotalPrice((product.getPrice() != null ? product.getPrice() : BigDecimal.ZERO).multiply(BigDecimal.valueOf(qty)));
            c.setNotes(notes.getText());
            c.setDeliveryDate(delivDatePicker.getValue() != null ? delivDatePicker.getValue().atStartOfDay() : null);

            service.update(c);
            popup.close();
            showInfo("Commande mise a jour", product.getName() + " x" + qty, "#16a34a", FontAwesomeSolid.CHECK_CIRCLE);
        });
        ...
        box.setStyle("-fx-background-color: white; -fx-background-radius: 16; -fx-border-color: #e0e0e0; -fx-border-radius: 16; -fx-effect: dropshadow(gaussian, rgba(0,0,0,0.15), 15, 0, 0, 4);");
    ...
    private void showCancelConfirm(Commande c) {
        ...
        btnYes.setStyle("-fx-background-color: #dc2626; -fx-text-fill: white; -fx-font-size: 13px; -fx-background-radius: 8; -fx-cursor: hand; -fx-padding: 8 20;");
        Button btnNo = new Button("Retour");
        btnNo.setStyle("-fx-background-color: #e5e7eb; -fx-text-fill: #333; -fx-font-size: 13px; -fx-background-radius: 8; -fx-cursor: hand; -fx-padding: 8 20;");
        ...
        btnYes.setOnAction(e -> {
            ...
            showInfo("Commande annulee", c.getCommandeNumber() != null ? c.getCommandeNumber() : "Commande #" + c.getId(), "#dc2626", FontAwesomeSolid.BAN);
        });
    }
    ...
    private void showDetails(Commande c) {
        ...
        close.setStyle("-fx-background-color: #1a73e8; -fx-text-fill: white; -fx-font-size: 13px; -fx-background-radius: 8; -fx-cursor: hand; -fx-padding: 6 30;");
    ...
    private Button actionBtn(FontAwesomeSolid iconType, String iconColor, String bgColor) {
        ...
        btn.setStyle("-fx-background-color: " + bgColor + "; -fx-cursor: hand; -fx-padding: 0; -fx-background-radius: 6;");
        return btn;
    }
    ...
    private void openPayment(Commande c) {
        ...
        if (!StripeConfig.isConfigured()) {
            showInfo("Paiement indisponible",
                    "Stripe n'est pas configure localement. Ajoute src/main/resources/stripe.properties pour activer le paiement.",
                    "#dc2626",
    ...
    private void downloadPdf(Commande c) {
        ...
        String defaultName = "Facture_" + (c.getCommandeNumber() != null ? c.getCommandeNumber() : ("CMD-" + c.getId())) + ".pdf";
        ...
        try {
            ...
            ex.printStackTrace();
            showInfo("Echec generation", ex.getMessage(), "#dc2626", FontAwesomeSolid.EXCLAMATION_TRIANGLE);
        }
    }
}
