...
public class PatientProduitsController {
    ...
    private void filter(String search) {
        if (search == null || search.trim().isEmpty()) {
            ...
            List<Produit> filtered = produitService.search(search).stream().filter(Produit::isActive).toList();
            renderGrid(filtered);
        }
    }
    ...
    private VBox buildCard(Produit p) {
        ...
        card.setStyle("-fx-background-color: white; -fx-background-radius: 14; -fx-border-color: #e5e7eb; -fx-border-radius: 14; -fx-effect: dropshadow(gaussian, rgba(0,0,0,0.06), 8, 0, 0, 2);");
        ...
        btnOrder.setStyle("-fx-background-color: #1a73e8; -fx-text-fill: white; -fx-font-size: 13px; -fx-background-radius: 8; -fx-cursor: hand; -fx-padding: 8;");
    ...
    private void showCommandeForm(Produit product) {
        ...
        Runnable recompute = () -> {
            if (Validators.isStrictlyPositiveInt(qtyField.getText())) {
                ...
                BigDecimal total = (product.getPrice() != null ? product.getPrice() : BigDecimal.ZERO).multiply(BigDecimal.valueOf(q));
        ...
        VBox box = new VBox(14, titleLbl, priceLbl, new Separator(), grid, labeled("Notes", notesArea), errorLbl, btns);
    ...
    private void openStripeCheckout(Commande c, Produit product) {
        if (!StripeConfig.isConfigured()) {
            showSuccess("Paiement indisponible", "Stripe n'est pas configure localement. La commande reste en attente de paiement dans 'Mes commandes'.");
            return;
        }
        try {
            ...
            ctrl.start(stage, c, product, paid -> {
                if (paid) {
                    showSuccess("Paiement reussi", product.getName() + " x" + c.getQuantity() + " - commande confirmee.");
                } else {
                    showSuccess("Paiement non finalise", "Tu peux payer plus tard depuis 'Mes commandes'.");
                }
                loadCatalogue();
            });
            stage.show();
            ...
            ex.printStackTrace();
            showSuccess("Erreur", "Impossible d'ouvrir le paiement : " + ex.getMessage());
        }
    }
}
